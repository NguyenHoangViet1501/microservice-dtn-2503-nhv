...
ALTER table users add column image_url varchar(200);

ALTER table users add column provider_id varchar(100);

ALTER table users add column provider ENUM('local', 'facebook', 'google', 'github') DEFAULT 'local';

alter table users modify role enum('USER','ADMIN','MANAGER') NULL;

alter table users modify password varchar(120) NULL;